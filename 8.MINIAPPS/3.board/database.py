...
def init_db(db):
    db.execute(
        "CREATE TABLE IF NOT EXISTS board (id INTEGER PRIMARY KEY AUTOINCREMENT, title VARCHAR(50), message VARCHAR(200))"
    )
    db.commit()
...
if __name__ == "__main__":
    ...
    # 미니 유닛 테스트
    db.execute(
        "CREATE TABLE IF NOT EXISTS board (id INTEGER PRIMARY KEY AUTOINCREMENT, title VARCHAR(50), message VARCAHAR(200))"
    )
