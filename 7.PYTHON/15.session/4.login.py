...
def profile():
    ...
    if request.method == "POST":
        ...
        if new_pw:
            # users 안에서 나의 비번을 바꾼다
            for u in users:
                if u['id'] == user['id']:
                    u['pw'] = new_pw
                    session['user'] = u # 세션에 저장된 사용자 정보도 업데이트 해준다 (세션은 서버측에 저장되므로, 세션에 저장된 사용자 정보도 업데이트 해줘야함)
                    break
            return render_template('profile.html', user=user,new_pw = new_pw, message="비밀번호가 성공적으로 변경되었습니다.")
